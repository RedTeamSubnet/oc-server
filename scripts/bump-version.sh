...
main()
{
	...
	local _pkg_files
	_pkg_files=$(find packages -name package.json -not -path "*/node_modules/*")
	for _pkg in $_pkg_files; do
		if grep -q '"version"' "$_pkg"; then
			sed -i '' "s/\"version\": \".*\"/\"version\": \"${_new_version}\"/" "$_pkg"
		fi
	done
